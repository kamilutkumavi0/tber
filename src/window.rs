...
fn window_framer(
    size: (u16, u16),
    _visual_line: Vec<String>,
    dict: HashMap<(u16, u16), char>,
) -> Vec<String> {
    // there is going to have a hash map for boxes
...
fn make_dict(app: &Vec<App>, size: (u16, u16)) -> HashMap<(u16, u16), char> {
    ...
    for i in app {
        for j in 0..size.1 {
            for k in 0..size.0 {
                if (j >= i.start.1 && j < i.start.1 + i.size.1)
                    && (k >= i.start.0 && k < i.start.0 + i.size.0)
                    && j - i.start.1 < i.visual_data.len() as u16
                {
                    if k - i.start.0 < i.visual_data[j as usize - i.start.1 as usize].len() as u16 {
                        let v = i.visual_data[j as usize - i.start.1 as usize]
                            .chars()
                            .nth(k as usize - i.start.0 as usize)
                            .unwrap();
                        temp.insert((k, j), v);
